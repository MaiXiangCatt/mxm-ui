...
describe('Input.vue', () => {
  test('render', () => {
    ...
    // 针对动态 class 的测试
    expect(wrapper.classes()).toContain('mxm-input')
    expect(wrapper.classes()).toContain('mxm-input--small')
    expect(wrapper.classes()).toContain('mxm-input--text')
    ...
    // 针对 slots 的测试
    expect(wrapper.find('.mxm-input__prepend').exists()).toBeTruthy()
    expect(wrapper.get('.mxm-input__prepend').text()).toBe('prepend')
  ...
  test('clearable', async () => {
    ...
    // 不应该出现 Icon 区域
    expect(wrapper.find('.mxm-input__clear').exists()).toBeFalsy()
    ...
    // 出现 Icon 区域
    expect(wrapper.find('.mxm-input__clear').exists()).toBeTruthy()

    // 点击 Icon 区域，触发 clear 事件
    await wrapper.get('.mxm-input__clear').trigger('click')
  ...
  test('toggle password', async () => {
    ...
    // 不应该出现 Icon 区域
    expect(wrapper.find('.mxm-input__password').exists()).toBeFalsy()
    const input = wrapper.get('input')

    expect(input.element.type).toBe('password')
    await input.setValue('123')

    const eyeIcon = wrapper.find('.mxm-input__password')
