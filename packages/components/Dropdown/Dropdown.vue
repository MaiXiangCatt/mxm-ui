<template>
  <div class="mxm-dropdown" :class="{ 'is-disabled': props.disabled}">
    <mxm-tooltip ref="tooltipRef"
                 v-bind="tooltipProps"
                 :virtural-triggering="splitButton"
                 :virtural-ref="triggerRef?.ref"
                 @visible-change="$emit('visible-change', $event)">
      
      <mxm-button-group v-if="splitButton" :type="type" :size="size" :disabled="disabled">
        <mxm-button @click="$emit('click', $event)">
          <slot name="default"></slot>
        </mxm-button>
        <mxm-button ref="triggerRef" icon="angle-down"></mxm-button>
      </mxm-button-group>
